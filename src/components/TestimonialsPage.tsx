...
const TestimonialsPage: React.FC = () => {
    ...
    return (
      <div className="max-w-6xl mx-auto px-6 py-16 pt-[150px]">
        <header className="mb-10 text-center">
          <h1 className="text-3xl md:text-4xl font-semibold text-gray-800">{t('testimonials.title')}</h1>
          <p className="mt-3 text-gray-600 max-w-2xl mx-auto">{t('testimonials.intro')}</p>
        </header>
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-16 pt-[150px]">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <aside className="w-full md:w-96 mt-4 md:mt-0">
            <div className="bg-white/6 p-4 rounded-xl shadow-soft">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-600">{current?.company} • {current?.year}</div>
                  </div>
                </div>
                <div className="text-right">
                  {current?.metrics && <div className="text-sm font-semibold text-green-300">{current.metrics}</div>}
                  <div className="mt-2 flex items-center justify-end gap-2">
                    <button onClick={prev} className="p-2 rounded-md bg-blue-100 text-blue-800 hover:bg-blue-200" aria-label="Previous">
                      <ChevronLeft size={18} />
                    </button>
                    <button onClick={next} className="p-2 rounded-md bg-blue-100 text-blue-800 hover:bg-blue-200" aria-label="Next">
              ...
              <blockquote className="mt-4 text-gray-700 text-lg">"{current?.quote}"</blockquote>

              <div className="mt-4">
                <label className="text-xs text-gray-600">{t('testimonials.filter.industry')}</label>
                <select
                  value={filter}
                  onChange={(e) => setFilter(e.target.value)}
                  className="w-full mt-2 rounded-md bg-white/80 text-gray-800 p-2 border border-gray-300"
              ...
              <div className="mt-4">
                <a href="#projects" className="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">{t('testimonials.case.view')}</a>
              </div>
            </div>
      ...
      <section>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">{t('testimonials.longform.title')}</h2>
            ...
            <div key={it.id} className="bg-white/5 p-4 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-600">{it.title} — {it.company}</div>
                </div>
                <div className="ml-auto text-sm text-green-300">{it.metrics}</div>
              </div>
              <p className="mt-3 text-gray-600">{it.quote}</p>
